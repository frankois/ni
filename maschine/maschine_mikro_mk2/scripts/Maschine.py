...
# Embedded file name: /Applications/Ableton Live 11 Suite.app/Contents/App-Resources/MIDI Remote Scripts/Maschine_Mikro_Mk2/Maschine.py
# Compiled at: 2021-04-30 12:09:45
# Size of source mod 2**32: 32190 bytes
...
class Maschine(ControlSurface):
        ...
        if not self._device_nav_button_right != None:
            ...
            if value != 0:
                ...
                if self.application().view.is_view_visible('Detail'):
                    self.application().view.is_view_visible('Detail/DeviceChain') or self.application().view.show_view('Detail')
                    self.application().view.show_view('Detail/DeviceChain')
                else:
                    direction = Live.Application.Application.View.NavDirection.right
                    self.application().view.scroll_view(direction, 'Detail/DeviceChain', not modifier_pressed)
    ...
    def send_to_display(self, text, grid=0):
        ...
        if len(text) > 28:
            text = text[:27]
        msgsysex = [
         240, 0, 0, 102, 23, 18, min(grid, 3) * 28]
...
# okay decompiling src/Maschine.pyc
