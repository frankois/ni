...
# Embedded file name: /Applications/Ableton Live 11 Suite.app/Contents/App-Resources/MIDI Remote Scripts/Maschine_Mikro_Mk2/ControlMode.py
# Compiled at: 2021-04-30 12:09:45
# Size of source mod 2**32: 26308 bytes
...
class ControlMode(MaschineMode):
    ...
    def _get_mixer_parm(self, track):
        if self._mode == CM_LEVEL:
            ...
            if self._selected_sends_index < nr_of_ret_tracks:
                if self._selected_sends_index != -1:
    ...
    def _do_track_selection(self, button_index):
        ...
        if self._in_clear_mode and track_sel._track:
            ...
            if button_index != self._track_sel_index:
                if track_sel._track:
                    current.turn_off()
                    track_sel.turn_on()
                    current = track_sel
    ...
    def _do_nav4(self, value):
        ...
        if value > 0:
            if self._mode == CM_SEND:
                nr_of_tracks = len(self.song().return_tracks)
                if not nr_of_tracks > 3 or self._selected_sends_index < 3 or self._selected_sends_index + 1 == nr_of_tracks:
...
# okay decompiling src/ControlMode.pyc
