...
# Embedded file name: /Applications/Ableton Live 11 Suite.app/Contents/App-Resources/MIDI Remote Scripts/Maschine_Mikro_Mk2/PadMode.py
# Compiled at: 2021-04-30 12:09:45
# Size of source mod 2**32: 13985 bytes
import Live
...
class PadMode(MaschineMode):
    ...
    def get_in_notes(self):
        ...
        if cs.has_clip:
            if cs.clip.is_midi_clip:
                in_notes = set()
                notes = cs.clip.get_notes_extended(0, 127, 0, cs.clip.length)
                for note in notes:
    ...
    def enter_clear_state(self):
        ...
        if cs != None and cs.has_clip and cs.clip.is_midi_clip:
            clip = cs.clip
            notes = clip.get_notes_extended(0, 127, 0, clip.length)
