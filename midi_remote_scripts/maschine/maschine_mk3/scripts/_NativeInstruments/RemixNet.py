...
# Embedded file name: /Applications/Ableton Live 11 Suite.app/Contents/App-Resources/MIDI Remote Scripts/_NativeInstruments/RemixNet.py
# Compiled at: 2021-05-19 16:14:41
...
class OSCEndpoint:
    ...
    def __init__(self, remoteHost, remotePort, localHost, localPort):
        """
    ...
    def send(self, address, msg):
        ...
        """
        osc_message = OSC.OSCMessage(address, msg)
        self.send_message(osc_message)

    def send_message(self, message):
        if message is not None:
            self.socket.sendto(message.get_binary(), self.remoteAddr)
    ...
    def process_incoming_udp(self):
        ...
        * The Ableton Live Python environment, although it includes the thread
        ...
        try:
            ...
            try:
                err = e
                if not hasattr(err, 'errno') or errno != errno.EAGAIN:
                    log('error handling message, errno ' + str(errno) + ': ' + str(e))
            finally:
                e = None
                del e
...
# okay decompiling src/_NativeInstruments/RemixNet.pyc
