...
# Embedded file name: /Applications/Ableton Live 11 Suite.app/Contents/App-Resources/MIDI Remote Scripts/Maschine_Mikro_Mk2/MidiEditSection.py
...
class MidiEditSection(CompoundComponent):
    ...
    def select_note(self, note):
        if self._current_clip:
            if self._note_set:
                self._note_index = 0
                self.apply_note_modifications(tuple(self._note_set[self._note_index]))
            ...
            if clip_slot:
                if clip_slot.has_clip:
                    if clip_slot.clip.is_midi_clip:
                        ...
                        self._note_set = clip.get_notes_extended(0, 127, 0, clip.length)
                        if self._note_index is None:
                            self._note_index = 0
                        else:
                            self._note_index = (self._note_index + 1) % len(self._note_set)
                        ls = []
                        selnote = self._note_set.node_id
                        ls.append(selnote)
                        clip.remove_notes_extended(selnote[1], selnote[0], selnote[2], 1)
                        clip.deselect_all_notes()
                        clip.apply_note_modifications(tuple(ls))
        ...
        if value != 0:
            ...
            if self._current_clip:
                ...
                for note in selected:
                    pitch = note[0]
                    self._current_clip.remove_notes_extended(pitch, 1, note[1], note[2])
